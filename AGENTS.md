...
Every `bash` code fence that contains an `az ...` command MUST be immediately followed by a `| Command | Purpose |` explanation table. The table lists the base command plus every long flag used, one row each. This is enforced by `scripts/validate_cli_explanations.py` (wired into the `Validate CLI Explanation Tables` CI job).
...
- **Oracle text review does NOT catch this.** Reviewing the Markdown source (which looks fine line-by-line) cannot reveal a rendering-level absorption bug. Rendered-HTML verification is required, not source review.
- A table-existence check is insufficient: a validator that only asserts "a table follows the fence" will pass a table that is missing its trailing blank line. The validator MUST also assert table **termination** (blank line or EOF). `find_unterminated_tables()` in `scripts/validate_cli_explanations.py` enforces this, and its doctests are the executable spec.
...
When generating or inserting explanation tables in bulk, always re-verify with the validator **and** spot-check the rendered HTML. When the fence-tracking of any scanner matters (KQL blocks use line-leading `|` such as `| where` / `| summarize`), the scanner MUST skip fenced code regions, or it will both over-count false positives and miss real defects.
